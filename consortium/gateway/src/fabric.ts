...
let connectionPromise: Promise<FabricConnection> | undefined;

function getTestNetworkPath(): string {
...
async function createConnection(): Promise<FabricConnection> {
...
const identityPath = path.join(orgPath, 'users', identityName, 'msp');
  ...
  path.join(identityPath, 'signcerts/cert.pem')
);
...
const keyDirectory = path.join(identityPath, 'keystore');
  ...
  const privateKey = createPrivateKey(await fs.readFile(
    path.join(keyDirectory, keyFilename)
  ));

  const tlsRootCertificate = await fs.readFile(path.join(
    orgPath,
...
export async function getDlsmContract(): Promise<Contract> {
  connectionPromise ??= createConnection();

  return (await connectionPromise).contract;
}

export async function closeFabricConnection(): Promise<void> {
